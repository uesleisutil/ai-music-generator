...
def warm_up_gpu(duration_minutes=60, profile="b3tr", region="us-east-1"):
    ...
    try:
        batch.update_compute_environment(
            computeEnvironment=compute_env, computeResources={"desiredVcpus": desired_vcpus}
        )
    ...
    print("⏳ Keeping instance warm... (Press Ctrl+C to scale down immediately)")
    try:
    ...
    # Scale down
    try:
        batch.update_compute_environment(computeEnvironment=compute_env, computeResources={"desiredVcpus": 0})
...
def main():
    parser = argparse.ArgumentParser(description="Warm up GPU instance for faster job execution")
    parser.add_argument(
        "--duration", type=int, default=60, help="Duration to keep instance warm (minutes, default: 60)"
    )
    parser.add_argument("--profile", type=str, default="b3tr", help="AWS profile name (default: b3tr)")
    parser.add_argument("--region", type=str, default="us-east-1", help="AWS region (default: us-east-1)")

    args = parser.parse_args()

    warm_up_gpu(duration_minutes=args.duration, profile=args.profile, region=args.region)
