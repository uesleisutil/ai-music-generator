...
def generate_with_bedrock_nova(prompt, output_path, width=1280, height=720):
    """Generate image using Amazon Nova Canvas via AWS Bedrock"""

    print("🎨 Generating image with AWS Bedrock (Amazon Nova Canvas)...")
    ...
    # Model ID for Amazon Nova Canvas
...
def generate_with_bedrock_titan(prompt, output_path, width=1280, height=720):
    """Generate image using Amazon Titan Image Generator v2 via AWS Bedrock"""

    print("🎨 Generating image with AWS Bedrock (Amazon Titan v2)...")
    print(f"   Resolution: {width}x{height}")
...
def enhance_prompt_for_lofi(prompt):
    ...
    # Lofi/anime style
    lofi_style = (
        "lofi aesthetic, anime art style, studio ghibli inspired, makoto shinkai style"
    )
    ...
    if "coffee" in prompt_lower or "cafe" in prompt_lower:
        ...
        scene = (
            "rainy night cityscape, rain drops on window, wet streets with reflections"
        )
        scene += (
            ", neon lights, moody atmosphere, purple and blue tones, cinematic lighting"
        )
        enhanced = f"{scene}, {prompt}, {lofi_style}, {base_quality}"
        ...
        # Generic lofi scene
        scene = (
            "lofi scene, detailed background, atmospheric lighting, depth, perspective"
        )
        enhanced = f"{scene}, {prompt}, {lofi_style}, {base_quality}"
...
def generate_image_bedrock(
    prompt, output_path="output/cover.png", model="nova", width=1280, height=720
):
    ...
    if model == "nova":
        return generate_with_bedrock_nova(prompt, output_path, width, height)
    elif model == "titan":
        return generate_with_bedrock_titan(prompt, output_path, width, height)
    elif model == "sdxl":
        # SDXL is deprecated, fallback to Nova
        print("⚠️  SDXL model is deprecated, using Nova Canvas instead")
        return generate_with_bedrock_nova(prompt, output_path, width, height)
    else:
        print(f"❌ Unknown model: {model}")
        print("💡 Available models: nova (recommended), titan")
        return None
...
def main():
    ...
    parser.add_argument(
        "--output", type=str, default="output/cover.png", help="Output path"
    )
    parser.add_argument(
        "--model",
        type=str,
        ...
        help="Bedrock model: nova (Amazon Nova Canvas - recommended), titan (Amazon Titan v2), sdxl (deprecated, will use nova)",
    )
    ...
    result = generate_image_bedrock(
        args.prompt, args.output, args.model, args.width, args.height
    )
